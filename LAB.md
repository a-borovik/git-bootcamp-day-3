...
Стек Pyton, `.gitignore` взят из прошлого задания

## Шаг 3. feat(tasks): complete_task

Добавили в `tasks.py`, тип `feat` и scope `tasks`

## Шаг 4. fix(tasks): валидация через add -p

Два не связанных изменения в одном файле. Что разделили через `git add -p`. Какие hunk-и видел Git, на какой ответили `y`, на какой `n`
...
Добавил в `README.md` (оглавление, секция TODO, code block, `<details>`, ссылка). Этот коммит сделали без `-m`, через редактор, с телом ≥ 2 строк.]
...
Выбрал `v0.1.0`, а не `v1.0.0`. На что ориентировались:
...
Почему annotated, а не lightweight: GitHub показывает в Releases только annotated, в annotated есть автор/дата/сообщение релиза.
...
Результат `git rev-parse v0.1.0` 6f86ad8b9a095b7e3d98ce1aa2507bbdeacd4133и `git rev-parse HEAD` da625861aab372210ddb05dfc3cbe817f0cef5ca
